...
public class OrderServiceImpl implements OrderService {
    ...
    public Order placeOrder(OrderInput orderInput, CartInput cartInput) {
        ...
        for (CartItemInput cartItem : cartInput.getCartItemInputs()) {
            ...
            // 재고가 부족한 경우에는 예외를 던진다
            if (afterStock < 0) {
                throw new StockShortageException("재고가 부족합니다.");
            }
            // 재고수량 업데이트
            product.setStock(afterStock);
            // 데이터베이스의 상품 데이터를 업데이트
            productRepository.update(product);
            // Repository에 넘겨줄 주문 상세 내역 오브젝트를 생성. 수량 등을 저장
