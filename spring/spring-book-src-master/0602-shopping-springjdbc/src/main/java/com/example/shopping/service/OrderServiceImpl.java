...
public class OrderServiceImpl implements OrderService {
    ...
    public Order placeOrder(OrderInput orderInput, CartInput cartInput) {
        // Repository에 넘겨줄 주문 객체 생성
        Order order = new Order();
        // ID는 UUID(Universally Unique Identifier. 랜덤한 ID를 생성할 때 이용되는 표준 기술)를 사용
        order.setId(UUID.randomUUID().toString());
        // 인수로 넘어온 주문 정보(OrderInput)의 정보를 Order 객체에 저장
        ...
        for (CartItemInput cartItem : cartInput.getCartItemInputs()) {
            ...
            //  데이터베이스의 상품 데이터를 갱신한다
            productRepository.update(product);
            // Repository에 넘겨줄 주문 내역 객체를 생성. 수량 등을 저
